...
In this tutorial, you'll learn how to use Swagger UI to parse through a sample Swagger specification document and display an output that looks similar to the [Swagger Petstore example](http://petstore.swagger.io/). The sample API will be the same weather API from Mashape [used earlier in this book](docapis_scenario_for_using_weather_api.html). For a more detailed conceptual overview of Swagger, see [Swagger Overview](pubapis_swagger_intro.html). For step-by-step tutorial on creating an OpenAPI specification document, see the [OpenAPI tutorial here](pubapis_openapi_tutorial_overview.html).
...
You can code this Swagger file by hand in a text editor, or you can auto-generate it from annotations in your source code. Different tools can consume the Swagger specification document to generate the interactive API documentation. In the following tutorial, I'll show you how to code the Swagger specification document by hand.
