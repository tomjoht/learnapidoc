...
*  **Using an open-source static site generator** like [Sphinx](pubapis_publishing_tool_options.html#sphinx), [Jekyll](pubapis_publishing_tool_options.html#sphinx), or [Hugo](pubapis_publishing_tool_options.html#sphinx) to build the files locally through the command line (rather than using a commercial program such as FrameMaker or Microsoft Word).
