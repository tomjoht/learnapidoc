...
One common feature (potentially a [design pattern](pubapis_design_patterns.html), but not observed widely enough) in API docs is a button or link that says "Edit on Github." For example, [Jekyll's docs](http://jekyllrb.com/docs/home/) include this link (titled "Improve this page"). This link takes users to the source file on Github, where they can edit the content. I'd seen this link/button on many developer doc sites, so I decided to give it a try with my own docs.
...
Here's what my docs looked like with the Edit on Github button:
