...
Some formal specifications &mdash; for example, Swagger (also called OpenAPI) and RAML &mdash; have been developed to describe REST APIs. When you describe your API using the [Swagger](pubapis_swagger.html) or [RAML](pubapis_raml.html) specification, tools that can read those specifications (like Swagger UI or the RAML API Console) will generate an interactive documentation output.
